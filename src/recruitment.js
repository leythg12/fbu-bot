...
async function createTicket(interaction, type) {
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`notify_${user.id}`).setLabel('📢 Prévenir le joueur').setStyle(ButtonStyle.Secondary),
  ...
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`close_${ticketChannel.id}`).setLabel('🔴 Fermer le ticket').setStyle(ButtonStyle.Danger),
  );
...
async function showPilotForm(interaction) {
  ...
  modal.addComponents(
    ...
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('network').setLabel('Réseau online (VATSIM/IVAO/aucun)').setStyle(TextInputStyle.Short).setRequired(false)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('motivation').setLabel('Expérience & motivation').setStyle(TextInputStyle.Paragraph).setRequired(true).setPlaceholder('Parle-nous de ton expérience en simulation et pourquoi tu veux rejoindre FBU...')),
  );
...
async function showStaffForm(interaction) {
  ...
  modal.addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('Prénom et Nom').setStyle(TextInputStyle.Short).setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('age').setLabel('Âge').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(3)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('poste').setLabel('Poste souhaité').setStyle(TextInputStyle.Short).setRequired(true).setPlaceholder('Modérateur, Responsable formation...')),
...
async function notifyPlayer(interaction, targetUserId) {
  ...
  if (!member) { await interaction.reply({ content: '❌ Membre introuvable.', ephemeral: true }); return; }
  const embed = new EmbedBuilder()
  ...
  try {
    ...
    await interaction.reply({ content: `❌ Impossible d'envoyer un MP (désactivés).`, ephemeral: true });
  }
}
...
async function acceptCandidate(interaction, targetUserId) {
  ...
  if (member) {
    ...
    const role = guild.roles.cache.get(cfg.ROLES.PILOT);
